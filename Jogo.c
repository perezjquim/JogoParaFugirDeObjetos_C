...
void verificarControlos(void)
{
		if(!Esquerda)				//Se for clicado para ir para a esquerda
		{
//			if(px > X_MIN)		//Se não tiver no limite esquerdo do DisplayY
//				moverJogadorEsquerda();	//Anda para a esquerda
			while(!Esquerda) {}		//Espera o 'largar' do botão
		}
		if(!Direita)				//Se for clicado para ir para a direita
		{
//			if(px < X_MAX)		//Se não tiver no limite direito do DisplayY
//				moverJogadorDireita();	//Anda para a direita
			while(!Direita) {}		//Espera o 'largar' do botão
		}
}
